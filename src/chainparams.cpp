...
{
    const char* pszTimestamp = "EternalGen Coin has been created 02112017";
...
public:
    CMainParams() {
        strNetworkID = "main";
        consensus.nSubsidyHalvingInterval = 840000; //Каждые 840000 вознаграждение за блок падает вдвое
        ...
        genesis = CreateGenesisBlock(1509610783, 2084524493, 0x1e0ffff0, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x"));
        assert(genesis.hashMerkleRoot == uint256S("0x"));

        // Note that of those with the service bits flag, most only support a subset of possible options сиды
        vSeeds.emplace_back("198.211.118.89", true);
        ...
        //    0.06     // * estimated number of transactions per second after that timestamp я тут был
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlock(1509610783, 293345, 0x1e0ffff0, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x"));
        assert(genesis.hashMerkleRoot == uint256S("0x"));
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top сиды
        vSeeds.emplace_back("198.211.118.89", true);
