...
Run with the -testnet option to run with "play eternals" on the test network, if you
...
EternalGen Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9013.
...
- BitcoinMiner : Generates eternals (if wallet is enabled).
...
However, in open source software such as EternalGen Core, where everyone uses
...
pay attention to for reviewers of EternalGen Core code.

General EternalGen Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: EternalGen Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of EternalGen Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against EternalGen Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:eternal-project/eternal.git
